...
function sortSchedule(scheduler: Scheduler): Scheduler {
    return scheduler.reduce((acc: Scheduler, schedule: Meeting) => {
        const startMin = timeToMinutes(schedule[0]);
        
        const idx = acc.findIndex(([start]) => timeToMinutes(start) > startMin);
        
        if (idx === -1) {
            acc.push(schedule);
        } else {
            acc.splice(idx, 0, schedule);
        }
...
function canSchedule(scheduler: Scheduler, newMeeting: Meeting): boolean {
    ...
    scheduler.push(newMeeting);
    scheduler = sortSchedule(scheduler);
